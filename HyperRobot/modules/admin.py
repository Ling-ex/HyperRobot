...
from HyperRobot.modules.helper_funcs.admin_rights import (
    user_can_changeinfo,
    user_can_promote,
)
...
def promote(update: Update, context: CallbackContext) -> str:
    ...
    if user_member.status in ("administrator", "creator"):
        message.reply_text(
            "Bagaimana saya bermaksud mempromosikan seseorang yang sudah menjadi admin?"
        )
        return
...
def lowpromote(update: Update, context: CallbackContext) -> str:
    ...
    if user_id == bot.id:
        message.reply_text(
            "Saya tidak bisa mempromosikan diri saya sendiri! Adminin dulu blok😒."
        )
        return
    ...
    try:
        ...
        if err.message == "User_not_mutual_contact":
            message.reply_text(
                "Saya tidak dapat mempromosikan seseorang yang tidak ada dalam grup, dasar lo buta map."
            )
        else:
            message.reply_text("Terjadi kesalahan saat mempromosikan.")
        return
...
def fullpromote(update: Update, context: CallbackContext) -> str:
    ...
    if user_member.status in ("administrator", "creator"):
        message.reply_text(
            "Bagaimana saya bermaksud mempromosikan seseorang yang sudah menjadi admin Tolol juga lu?"
        )
        return

    if user_id == bot.id:
        message.reply_text(
            "Saya tidak bisa mempromosikan diri saya sendiri! Adminin Dulu blok tolol😒."
        )
        return
    ...
    try:
        ...
        if err.message == "User_not_mutual_contact":
            message.reply_text(
                "Saya tidak dapat mempromosikan seseorang yang tidak ada dalam grup, goblok juga anak satu ini😂."
            )
        else:
            message.reply_text(
                "Terjadi kesalahan saat mempromosikan, salah lu bukan salah gua."
            )
        return
...
def demote(update: Update, context: CallbackContext) -> str:
    ...
    if user_member.status == "creator":
        message.reply_text(
            "Orang ini MENCIPTAKAN obrolan, bagaimana cara saya menurunkannya? Mikir lah goblok"
        )
        return

    if not user_member.status == "administrator":
        message.reply_text("Tidak dapat menurunkan apa yang tidak dipromosikan goblok!")
        return

    if user_id == bot.id:
        message.reply_text(
            "Saya tidak bisa menurunkan diri saya sendiri! Adminin dulu goblok😒."
        )
        return
...
def set_title(update: Update, context: CallbackContext):
    ...
    if not title:
        message.reply_text("Mengatur judul kosong tidak melakukan apa-apa tolol😂!")
        return
    ...
    try:
        ...
        message.reply_text(
            "Entah mereka tidak dipromosikan oleh saya atau Anda menetapkan teks judul yang tidak mungkin untuk disetel goblok😂."
        )
        return
        ...
        try:
            ...
            msg.reply_text(
                ...
                reply_markup=InlineKeyboardMarkup(
                    [[InlineKeyboardButton("👉 Pergi ke pesan", url=f"{message_link}")]]
                ),
                parse_mode=ParseMode.HTML,
...
def unpin(update: Update, context: CallbackContext):
    ...
    if (
        ...
        message.reply_text(
            "Anda tidak memiliki hak yang diperlukan untuk melakukan itu tomlol!"
        )
        return
    ...
    if prev_message and is_group:
        try:
            ...
            msg.reply_text(
                f"Lepas pin <a href='{message_link}'>this message</a>.",
                parse_mode=ParseMode.HTML,
                disable_web_page_preview=True,
            )
        except BadRequest as excp:
    ...
    if not prev_message and is_group:
        try:
            ...
            msg.reply_text("Lepas pin dari pesan yang disematkan terakhir.")
        except BadRequest as excp:
            if excp.message == "Pesan untuk melepas pin tidak ditemukan":
                msg.reply_text(
                    "Saya tidak dapat melihat pesan yang disematkan, Mungkin sudah dilepas pinnya, atau pin Pesan ke yang lama😒"
                )
            else:
                raise
    ...
    if chat.username:
        ...
        if bot_member.can_invite_users:
            ...
            update.effective_message.reply_text(
                "Saya tidak memiliki akses ke tautan undangan, coba ubah izin saya menjadi pemilik🗿!",
            )
    else:
        update.effective_message.reply_text(
            "Saya hanya bisa memberi Anda tautan undangan untuk supergrup dan saluran, maaf ya blok!",
        )
...
def bug_reporting(update: Update, _: CallbackContext):
    ...
    try:
        ...
        bot.sendMessage(
            chat.id,
            f"✅Bug Anda telah dikirim ke  <b>Bot Admins</b>. Thanks for reporting the bug.",
            parse_mode=ParseMode.HTML,
        )
...
*Perintah Berikut adalah Admin saja:* 
❂ /pin*:* diam-diam menyematkan pesan yang dibalas - add `'loud'` atau `'tell'` untuk memberikan pemberitahuan kepada pengguna
...
❂ /setgtitle <text>*:* tentukan judul grup
