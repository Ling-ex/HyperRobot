...
from HyperRobot import telethn, OWNER_ID, DEV_USERS, DRAGONS, DEMONS
...
async def rm_deletedacc(show):
    ...
    if con != "clean":
        ...
        if del_u > 0:
            del_status = (
                f"**Founding** `{del_u}` **Hapus member goblok yg suka deak deak akun/Zombie Dari dunia ini,"
    ...
    if not admin and not creator:
        return await show.reply("**LU BUKAN ADMIN TOLOL!**")
    memek = await show.reply("`Menghapus aku depresi...`")
    del_u = 0
    del_a = 0
    ...
    if del_a > 0:
        del_status = (
            ...
            f"\n`{del_a}` **Group aman tidak ada akun depresi.**"
        )
